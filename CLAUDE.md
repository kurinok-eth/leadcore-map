...
### 3. Дедупликация данных из Битрикс

Битрикс может отдавать одного представителя **несколькими записями** с разными `regionId` (например, Таранова: одна запись с СФО, другая с ДФО). В `App.tsx` при загрузке данных записи с одинаковым `id` объединяются в одну с массивом `regionId`. Это критически важно — без дедупликации дублируются React-ключи (`key={rep.id}`), что ломает reconciliation и вызывает "призрачные" карточки при фильтрации.
