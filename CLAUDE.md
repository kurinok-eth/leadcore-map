...
- `src/components/ListView.tsx` — табличный список с поиском по имени, городу, региону, округу
...
- `src/cityMapping.ts` — маппинг названий городов → ID регионов для поиска (город → RU-XXX)
...
**КРИТИЧЕСКИ ВАЖНО:** GeoJSON координаты `[lon, lat]` преобразуются в SVG через проекцию Меркатора в `src/utils.ts`. Функции `projectPoint()`, `polygonToPath()`, `multiPolygonToPath()`, `geometryToPath()` — трогать осторожно. SVG viewBox: `0 0 1000 600`, границы карты: lon 19–180, lat 41–82.
...
- Смешанно (округ + отдельный регион): `regionId: ['ПФО', 'RU-AMU']`
...
### 3. Парсинг и дедупликация данных из Битрикс

Поле `REGION_ID` в Битрикс — список с **множественным выбором**. При экспорте значения разделяются через `;` (например: `ЦФО;RU-AMU`). В `App.tsx` функция `parseRegionIds()` разбивает строку по `;`, `,` или `\n` в массив перед дедупликацией.

Битрикс также может отдавать одного представителя **несколькими записями** с разными `regionId`. В `App.tsx` записи с одинаковым `id` объединяются в одну с массивом `regionId`. Без дедупликации дублируются React-ключи (`key={rep.id}`), что ломает reconciliation.

**Значения поля `REGION_ID` в Битрикс:** в колонке **"Значение"** (не XML_ID!) должен стоять код — либо аббревиатура округа (`ЦФО`), либо код региона (`RU-MOW`). Именно это значение читает `component.php` через `PROPERTY_REGION_ID_VALUE` и передаёт в React.

### 4. Поиск по городам (cityMapping.ts)

`ListView.tsx` поддерживает поиск по названиям городов. Маппинг город→регион хранится в `src/cityMapping.ts`. **Важно:** многие города НЕ являются подстрокой названия региона из-за различий в словоформах:
- "Тула" ≠ подстрока "Тульская" (тул**а** vs тул**ь**ская)
- "Рязань" ≠ "Рязанская", "Пермь" ≠ "Пермский", "Самара" ≠ "Самарская" и т.д.

При добавлении нового города проверять: `"название_региона".toLowerCase().includes("название_города")`. Если `false` — город нужно добавить в `cityMapping.ts`.
...
4. **Федеральные округа и регионы**
   - Представитель с `regionId: 'ЦФО'` виден во ВСЕХ регионах ЦФО
   - Смешанный массив работает: `regionId: ['ПФО', 'RU-AMU']` — виден в ПФО + Амурской области
   - Москва (`RU-MOW`) и МО (`RU-MOS`) — независимые регионы, можно назначать отдельно
...
- `component-ready.php` — **боевой** файл (iblockId=57), заливать на сервер как `component.php`
- `component.php` — шаблон с TODO для iblockId
...
- `bitrix-component-example/component-ready.php` → `/local/components/custom/russia.map/component.php` (переименовать!)

GeoJSON (`public/russia-regions.geojson`) менять редко — заливать только при изменении границ регионов.
